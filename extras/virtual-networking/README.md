...
- **fixtures** expanded from `fixtures/*.json` templates declared by topologies
- **services** from command-backed service actions
...
- network operations
- semantic roles
- ordered fixture template specifications

At runtime, namespace defaults such as `LAN_NS` and `NODE_NS` are prefixed
with the run ID, so concurrent CI jobs do not share or delete the same network
namespaces. The original role names stay available through bindings such as
`{ "role": "node" }`.
The top-level `namespaces` list drives namespace creation during setup and
deletion during cleanup; topology `operations` only wire and configure those
namespaces. Topology `fixtures` declare infrastructure layered onto that
network shape, such as local bootstrap nodes and miniupnpd routers.
...
templates live in `fixtures/*.json`; topology entries choose a template `type`
and `amount`. Current fixture types are:

- `local-bootstrap`
  - starts a local OpenDHT bootstrap node in a role namespace
- `local-bootstrap-ipv6`
  - starts a local IPv6 OpenDHT bootstrap node in a role namespace
- `miniupnpd`
...
Topology fixture entries use one object shape:

```json
{ "type": "miniupnpd", "amount": 3 }
```

`amount` controls how many instances the template expands from the topology's
ordered roles and defaults. For `miniupnpd`, instance names come from the
template's name pattern and the router role slug; for example, role
`node_b_router` expands to `miniupnpd-node-b-router`.
...
under `lib/fixture_actions/`, not in Python runtime branches.
...
- `command`
  - launches an argv array in a role namespace
  - supports file readiness and structured JSON outputs
  - uses reusable helpers under `lib/service_actions/`
...
| `kind` | yes | Service kind, currently `command` |
| `role` | yes | Topology role where the service runs |
| `argv` | yes | Command argv array, with context placeholders |
| `env` | no | Extra command environment values |
| `readiness` | no | Readiness object, currently `none` or `file` |
| `outputs` | no | Output contract, currently `none` or JSON |
