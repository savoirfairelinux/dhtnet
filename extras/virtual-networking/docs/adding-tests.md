...
- a **topology** from `topologies/*.json`, including its fixtures
...
Topologies define the network shape, defaults, roles, namespaces, operations,
and infrastructure fixtures.
...
The top-level `namespaces` list drives both setup namespace creation and cleanup. `operations` should only wire and configure those namespaces. Put infrastructure fixtures, such as miniupnpd routers and local bootstrap nodes, in the topology's `fixtures` list.
...
3. Reuse an existing topology, service set, and probe names.
