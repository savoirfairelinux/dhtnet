...
- optional long-running **services** located in `services/`
...
| A new command/check that fits existing runtime behavior | a new `probes/*.json` file, and maybe a small typed action module in `lib/tools/probe_actions/` |
...
| A new fixture kind, service kind, or probe action type | Python/runtime changes in `lib/runtime/`, `lib/loaders/`, or `lib/tools/probe_actions/` plus the new JSON definition(s) |
...
Services write structured outputs that later steps can consume.

### Probes

Probes are reusable step definitions in `probes/*.json`. Each probe carries an ordered `probe_sequence`
of typed actions implemented by modules under `lib/tools/probe_actions/`, so the
...
Action modules are auto-loaded from `lib/tools/probe_actions/`. To add one, create a module with a `register_actions()` function and register each action with `register_probe_action(...)`. The module name must not start with `_`.
...
4. Prefer composing existing typed actions; add a small new action module in `lib/tools/probe_actions/` only when the probe needs a new assertion semantic.
...
5. Consume its outputs from service/bootstrap or probe inputs if needed.
...
| a new service kind | `lib/runtime/service_runtime.py` |
| a new probe action type | a module under `lib/tools/probe_actions/` and maybe `lib/tools/probe_runner.py` if the framework changes |
