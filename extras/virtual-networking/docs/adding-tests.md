...
| A new command/check that fits existing runtime behavior | a new `probes/*.json` file, and maybe a small typed action in `lib/tools/probe_runner.py` |
...
| A new fixture kind, actor kind, or probe action type | Python/runtime changes in `lib/runtime/`, `lib/loaders/`, or `lib/tools/probe_runner.py` plus the new JSON definition(s) |
...
of typed actions implemented by `lib/tools/probe_runner.py`, so the
...
4. Prefer composing existing typed actions; add a small new action in `lib/tools/probe_runner.py` only when the probe needs a new assertion semantic.
...
| a new probe action type | `lib/tools/probe_runner.py` and maybe `lib/loaders/context_loader.py` if the schema changes |
