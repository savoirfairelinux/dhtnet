...
generates a temporary identity, emits the actor peer ID, starts `dsh -l -a`,
...
ACTOR_OUTPUT_CLI="${VNET_ROOT}/lib/tools/actor_output.py"
...
if [[ -n "${VNET_ACTOR_READY_FILE:-}" ]]; then
    ready_timeout_s="${VNET_ACTOR_READY_TIMEOUT_S:-60}"
    if [[ ! "${ready_timeout_s}" =~ ^[0-9]+$ ]]; then
        echo "Error: VNET_ACTOR_READY_TIMEOUT_S must be a non-negative integer" >&2
        exit 1
    fi
    ...
    if [[ ! -e "${VNET_ACTOR_READY_FILE}" ]]; then
