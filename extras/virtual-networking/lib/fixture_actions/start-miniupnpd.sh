...
cleanup_on_error() {
    if [[ -n "${VNET_FIXTURE_CONFIGFILE:-}" && -s "${VNET_FIXTURE_CONFIGFILE}" ]]; then
        printf 'miniupnpd config:\n' >&2
        sed 's/^/  /' "${VNET_FIXTURE_CONFIGFILE}" >&2 || true
    fi
    if [[ -n "${VNET_FIXTURE_LOGFILE:-}" && -s "${VNET_FIXTURE_LOGFILE}" ]]; then
        printf 'miniupnpd log:\n' >&2
        sed -n '1,160p' "${VNET_FIXTURE_LOGFILE}" | sed 's/^/  /' >&2 || true
    fi
