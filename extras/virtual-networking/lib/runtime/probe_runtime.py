...
from lib.probe_actions import register_default_probe_actions
...
def resolve_probe_binding(
    ...
    *,
    ...
    if "role" in binding:
        ...
        service_name = require_string(binding["service"], field_name=binding_path, scenario_path=scenario.path)
        field_name = binding.get("field", "namespace")
        if not isinstance(field_name, str) or not field_name:
            raise ScenarioError(f"{scenario.path}: {binding_path}.field must be a non-empty string when binding a service")
        context_key = service_context_key(service_name, field_name)
...
def build_default_probe_inputs(
    ...
    *,
    ...
    if "DHTNET_BOOTSTRAP_HOST" in context:
        defaults["bootstrap_host"] = context["DHTNET_BOOTSTRAP_HOST"]
    if "DHTNET_BOOTSTRAP_PORT" in context:
        defaults["bootstrap_port"] = context["DHTNET_BOOTSTRAP_PORT"]
    return defaults
