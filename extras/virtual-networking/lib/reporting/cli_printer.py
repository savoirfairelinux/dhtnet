...
def print_dry_run(scenario: ScenarioSpec) -> None:
    ...
    if scenario.services:
        ...
        for service in scenario.services:
            ...
            extra = f", bootstrap_fixture={service.bootstrap_fixture}" if service.bootstrap_fixture else ""
            print(f"  - {service.name}: {service.kind} in role {service.role} ({service_role_ns}), wait={service.wait_s:.1f}s{extra}")
    print("Steps:")
