...
from lib.core.paths import DEFAULT_ARTIFACT_ROOT
...
def print_dry_run(scenario: ScenarioSpec) -> None:
    ...
    if topology.roles:
        ...
        for role in sorted(topology.roles.values(), key=lambda item: item.name):
            ...
            print(f"  - {role.name}: {namespace}")
    if topology.fixtures:
        print("Fixtures:")
        for fixture in topology.fixtures:
            print(f"  - {fixture.name}: {fixture.kind}")
    if scenario.services:
        print("Services:")
        for service in scenario.services:
            service_role_ns = context[role_context_key(service.role, "namespace")]
            readiness = service.readiness.type
            if service.readiness.timeout_s:
                readiness = f"{readiness}/{service.readiness.timeout_s:.1f}s"
            print(
                f"  - {service.name}: {service.kind} in role {service.role} "
                f"({service_role_ns}), readiness={readiness}"
            )
    print("Steps:")
