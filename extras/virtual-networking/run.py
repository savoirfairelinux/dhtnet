...
def launch_actor(
    ...
    *,
    ...
    try:
        ...
        pass
    argv = build_actor_argv(namespace, launch_command, launch_user, ready_path=ready_path)
    ...
    if exit_code is None:
        if actor.ready_path is not None and not actor.ready_path.exists():
            return False, (
                ...
                f"but it did not signal readiness. Log: captures/{actor.log_path.name}"
            )
        return True, (
...
def load_scenario(path: Path) -> ScenarioSpec:
    ...
    actor_command = data.get("actor_command")
    if actor_command is not None and not isinstance(actor_command, str):
        raise ScenarioError(f"{path}: actor_command must be a string when present")
...
def print_dry_run(scenario: ScenarioSpec) -> None:
    ...
    if scenario.actor_command or scenario.actor_namespace:
        print(f"Actor command: {scenario.actor_command or '<none>'}")
...
def run_scenario(
    ...
    *,
    ...
    try:
        ...
        if not should_exit:
            needs_actor = bool(scenario.actor_command)
            if needs_actor:
                actor_command_template = scenario.actor_command
                if not actor_command_template:
                    recorder.assertion(
                        "launch_actor",
                        "failed",
                        0,
                        "Scenario requires an actor_command but none was configured.",
                    )
                    status = "failed"
                    should_exit = True
                else:
                    ...
                    if not actor_namespace_template:
                        ...
                        actor_command = resolve_text(
                            actor_command_template,
                            context,
                            scenario_name=scenario.name,
                        )
                        started_ms = now_ms()
