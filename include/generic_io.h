...
public:
     ...
     * @return Number of bytes processed, zero or negative value to close the socket.
     * A negative value indicates an error, zero indicates a graceful shutdown.
     * In both cases the channel is shutdown immediately and synchronously,
     * guaranteeing that no further channel callback is called,
     * except the shutdown callback if set afterwards.
     */
    using RecvCb = std::function<ssize_t(const ValueType* buf, std::size_t len)>;

    /**
     * Close established connection.
     * @note Terminate outstanding blocking read operations with an empty error code, but a 0 read size.
     */
     ...
     * @note The address is not valid (addr.isUnspecified() returns true) if it's not known
     * or not available.
     */
