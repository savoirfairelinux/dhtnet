...
dhtnc_params parse_args(int argc, char** argv){
    dhtnc_params params;
    int opt;
...
int main(int argc, char** argv){
    ...
    std::unique_ptr<dhtnet::DhtNc> dhtnc;
    if (params.listen) {
        fmt::print("DhtNC 0.6\n");
        fmt::print("Loaded identity: {}\n", identity.second->getId());
        // create dhtnc instance
        dhtnc = std::make_unique<dhtnet::DhtNc>(identity, params.bootstrap_ip, params.bootstrap_port);
    } else {
        dhtnc = std::make_unique<dhtnet::DhtNc>(identity, params.bootstrap_ip, params.bootstrap_port, params.peer_id, params.port, params.ip_add);
    }
    dhtnc->run();
}
