...
bool
runConnection(std::shared_ptr<asio::io_context> ioContext,
        std::shared_ptr<std::thread> ioContextRunner,
        std::shared_ptr<IceTransportFactory>& factory,
        ...
        std::shared_ptr<Logger> logger)
{
    ...
    std::condition_variable dhtConVar;
    ...
    dht->connectionManager->connectDevice(peer_id, "channelName", [&](std::shared_ptr<ChannelSocket> socket, const dht::InfoHash&) {
        if (socket) {
            ret = true;
            cv.notify_one();
        }
    ...
    return cv.wait_for(lock, 5s, [&] { return ret; });
}
...
int main(int argc, char** argv)
{
    ...
    while (true) {
        ...
        if (command == "quit" || command == "exit" || command == "q" || command == "x")
            ...
            if (dhtnet::runConnection(ioContext, ioContextRunner, factory, peer_id, logger))
                fmt::print("Connected\n");
            else
