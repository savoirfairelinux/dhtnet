...
IceTransport::Impl::initIceInstance(const IceTransportOptions& options)
{
    ...
    upnpMappingTimeout_ = std::max(options.upnpMappingTimeout, std::chrono::milliseconds::zero());
...
IceTransport::Impl::requestUpnpMappings()
{
    ...
    for (unsigned id = 1; id <= compCount_; id++) {
        ...
        // Request the mapping
        auto map = upnp_->reserveMapping(requestedMap);
        if (not map) {
            // Without a mapping for every component, waiting is useless.
            std::lock_guard lockMapping(state->mutex);
            state->failed = true;
            break;
        }
...
IceTransport::send(unsigned compId, const unsigned char* buf, size_t len)
{
    ...
    if (status == PJ_EPENDING && isTCPEnabled()) {
        ...
        // If an error occurred, PJSIP will set the 'sent' parameter of the on_data_sent()
        // callback equal to the negation of the error code.
        if (sent < 0) {
        ...
        // bytes length).
        if (sent != len + 2) {
            // This should never happen. If no error occurred, then PJSIP only calls on_data_sent() for a
            // given write operation if all the data has been written (see ioqueue_dispatch_write_event).
            // Getting an incorrect number of bytes here most likely means that we got the callback for a
            // different operation than the one we were waiting for, thus indicating a bug either in DHTNet
            // or in PJSIP itself.
            if (pimpl_->logger_)
