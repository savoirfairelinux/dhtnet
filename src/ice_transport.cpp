...
IceTransport::Impl::requestUpnpMappings()
{
    ...
    for (unsigned id = 1; id <= compCount_; id++) {
        ...
        // Enable auto update to automatically update the mapping if it fails, during the PORT_MAPPING_TIMEOUT.
        requestedMap.enableAutoUpdate(true);

        requestedMap.setNotifyCallback([upnpMappings, requestedMap, this](Mapping::sharedPtr_t mapPtr) {
    ...
    cv.wait_for(lock, PORT_MAPPING_TIMEOUT, [&] {
        return  upnpMappings->size() == compCount_;
    });
    ...
    if (upnpMappings->size() != compCount_){
        ...
        // disable the auto update for all mappings: after the ICE candidates are gathered, we don't need to update the mappings
        // and remove the notify callback
        for (auto& map : *upnpMappings){
            map.second->enableAutoUpdate(false);
