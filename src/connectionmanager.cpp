...
createConfig(std::shared_ptr<ConnectionManager::Config> config)
{
    if (!config->certStore){
        config->certStore = std::make_shared<dhtnet::tls::CertificateStore>("client", config->logger);
    }
    if (!config->dht) {
        dht::DhtRunner::Config dhtConfig;
        dhtConfig.dht_config.id = config->id;
        dhtConfig.threaded = true;
        dht::DhtRunner::Context dhtContext;
        dhtContext.certificateStore = [c = config->certStore](const dht::InfoHash& pk_id) {
            std::vector<std::shared_ptr<dht::crypto::Certificate>> ret;
            if (auto cert = c->getCertificate(pk_id.toString()))
                ret.emplace_back(std::move(cert));
            return ret;
        };
        config->dht = std::make_shared<dht::DhtRunner>();
        config->dht->run(dhtConfig, std::move(dhtContext));
        config->dht->bootstrap("bootstrap.jami.net");
    }
    if (!config->factory){
        config->factory = std::make_shared<IceTransportFactory>(config->logger);
    }
    return config;
}
