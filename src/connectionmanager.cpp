...
struct ConnectionInfo
{
    ...
    getInfo(const DeviceId& deviceId, dht::Value::Id valueId) const
    {
...
struct DeviceInfo {
    ...
    getConnectionList() const {
        ...
        for (auto& [id, ci] : info) {
            ...
            ret.emplace_back(ci->getInfo(deviceId, id));
        }
                                       ...
                                       const std::string& connType)
{
    ...
    dht::ThreadPool::computation().run([w = weak_from_this(),
        ...
        dht::Value::Id vid;
        {
            std::lock_guard<std::mutex> lk(sthis->randMtx_);
        ...
        if (auto info = di->getConnectedInfo()) {
            ...
            if (info->socket_) {
                ...
                sthis->sendChannelRequest(di, info->socket_, name, vid);
                return;
            }
        }
...
ConnectionManager::getConnectionList(const DeviceId& device) const
{
    ...
    if (device) {
        if (auto deviceInfo = pimpl_->infos_.getDeviceInfo(device)) {
            connectionsList = deviceInfo->getConnectionList();
        }
    } else {
