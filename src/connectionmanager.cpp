...
ConnectionManager::Impl::loadTreatedMessages()
{
    ...
    treatedMessages_ = fileutils::loadIdList(path);
}

void
ConnectionManager::Impl::saveTreatedMessages()
{
    dht::ThreadPool::io().run([w = weak_from_this()]() {
        if (auto sthis = w.lock()) {
            auto& this_ = *sthis;
            fileutils::check_dir(this_.config_->cachePath);
            std::lock_guard<std::mutex> lock(this_.messageMutex_);
            fileutils::maintainIdList(this_.treatedMessages_);
            fileutils::saveIdList(this_.config_->cachePath / "treatedMessages", this_.treatedMessages_);
        }
    });
}
...
{
    std::lock_guard<std::mutex> lock(messageMutex_);
    auto res = treatedMessages_.emplace(id, std::chrono::system_clock::now());
    if (res.second) {
        saveTreatedMessages();
        return false;
    }
