...
NatPmp::terminate()
{
    ...
    // Capture shared_from_this() to keep the object alive if the lambda
    // executes after terminate() returns on timeout.
    auto self = std::static_pointer_cast<NatPmp>(shared_from_this());
