...
using NodeListPtr = std::unique_ptr<IXML_NodeList, IXML_NodeListDestructor>;
...
{
    NodeListPtr nodeList(ixmlDocument_getElementsByTagName(doc, item));
    if (nodeList) {
...
{
    NodeListPtr nodeList(ixmlElement_getElementsByTagName(element, item));
    if (nodeList) {
        // If there are several nodes which match the tag, we only want the first one.
        return getElementText(ixmlNodeList_item(nodeList.get(), 0));
    }
...
PUPnP::parseIgd(IXML_Document* doc, std::string locationUrl)
{
    ...
    NodeListPtr serviceList(ixmlDocument_getElementsByTagName(doc, "serviceType"));
