...
static const constexpr struct option long_options[] = {{"help", no_argument, nullptr, 'h'},
                                                       ...
                                                       {"tap_device", required_argument, nullptr, 'd'},
                                                       {"tap_ip", required_argument, nullptr, 'a'},
                                                       {"tap_netmask", required_argument, nullptr, 'm'},
                                                       {nullptr, 0, nullptr, 0}};
...
parse_args(int argc, char** argv)
{
    ...
    while ((opt = getopt_long(argc, argv, "hvlw:r:u:t:I:b:a:d:m:", long_options, nullptr)) != -1) {
        ...
        case 'r':
            ...
            break;
        case 'd':
            params.tap_device = optarg;
            break;
        case 'a':
            params.tap_ip = optarg;
            break;
        case 'm':
            params.tap_netmask = optarg;
            break;
        default:
    ...
    if (params.tap_device.empty())
        params.tap_device = "tap0";
    if (params.tap_ip.empty())
        params.tap_ip = "10.66.77.0";
    if (params.tap_netmask.empty())
        params.tap_netmask = "255.255.255.0";
    return params;
}
...
main(int argc, char** argv)
{
    ...
    if (params.help ){
          fmt::print("Usage: dvpn [options] [PEER_ID]\n"
               ...
               "  -d, --tap_device      Specify the tap_device option with an argument.\n"
               "  -a, --tap_ip          Specify the tap_ip option with an argument.\n"
               "  -m, --tap_netmask     Specify the tap_netmask option with an argument.\n");
        return EXIT_SUCCESS;
    }
    if (params.version) {
        fmt::print("dvpn v1.0\n");
    ...
    if (params.listen) {
        ...
        dvpn = std::make_unique<dhtnet::Dvpn>(params.path, identity, params.bootstrap, params.turn_host, params.turn_user, params.turn_pass, params.turn_realm, params.tap_device, params.tap_ip, params.tap_netmask);
    } else {
