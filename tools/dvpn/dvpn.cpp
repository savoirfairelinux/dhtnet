...
Config(const YAML::Node& node, char* tun_device)
{
    ...
    if (node["ip_address"])
        ...
        if (node["netmask"])
            netmask = node["netmask"].as<std::string>();
        if (node["script_path"])
...
// Call a script shell
int
call_script_shell(const char* script, const char* remote_tun_ip, const char* tun_ip, const char* netmask, const char* tun_device, const char* remote_address, bool is_client)
{
    pid_t pid;
    int status;
    std::mutex mtx;
