...
dhtnet::Dvpn::Dvpn(const std::filesystem::path& path,
                   ...
                   const std::string& tun_netmask ): ioContext(std::make_shared<asio::io_context>())
{
    ...
    if (configure_ip_address(strdup(tun_device.c_str()), strdup(tun_ip.c_str()), strdup(tun_netmask.c_str())) < 0) {
        if (logger)
    ...
    connectionManager->onConnectionReady([&,tun_stream](const DeviceId&,
...
dhtnet::Dvpn::Dvpn(const std::filesystem::path& path,
                   ...
                   const std::string& tun_device,
                   const std::string& tun_ip,
                   const std::string& tun_netmask) : Dvpn(path, identity, bootstrap,turn_host,turn_user,turn_pass, turn_realm, tun_device, tun_ip, tun_netmask)
{
